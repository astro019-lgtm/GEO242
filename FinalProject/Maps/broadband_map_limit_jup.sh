...
#create map file
outfile='broadband_limit_map.ps'
...
bbstations='broadbands.csv'

#gradient file
grad='Maps/grad_OV.i2.dem'

#longest pair
l_pair='bb_row_longest.csv'

#shortest pair
s_pair='bb_row_shortest.csv'
...
awk -F',' '{print $2,$3}' $bbstations | gmt psxy -J -R -Sc0.2 -Gblue -O -K >> $outfile

#get station pairs by looping through all of them and it came out BEAUTIFUL
awk -F',' '{ x[NR]=$2; y[NR]=$3; n=NR } END{ for(i=1;i<=n;i++) for(j=i+1;j<=n;j++) print x[i],y[i] "\n" x[j],y[j] "\n" }' $bbstations | gmt psxy -J -R -W0.1p,lightblue -O -K >> $outfile

#plot the longest station pairs
awk -F',' '{print $1,$2}' $l_pair | gmt psxy -J -R -Sc0.4 -Gpurple -O -K >> $outfile
awk -F',' '{print $1,$2}' $l_pair | gmt psxy -J -R -W0.7p,magenta -O -K >> $outfile

#plot the shortest station pairs
awk -F',' '{print $1,$2}' $s_pair | gmt psxy -J -R -Sc0.4 -Gseagreen3 -O -K >> $outfile
awk -F',' '{print $1,$2}' $s_pair | gmt psxy -J -R -W0.7p,green1 -O -K >> $outfile
...
echo -118.37 37.37 Bishop | gmt pstext -J -R -F+f7p,Helvetica+jBC -Dj0.3 -Gwhite -Wthin,hotpink -O -K >> $outfile
#lone pine
...
#create legend
gmt psscale -J -R -C$colpal -Dg-119.4/35.6+jBL+w4c/0.75c+v -Bpa1000f200+l"Elevation(m)" -O -K >> $outfile
